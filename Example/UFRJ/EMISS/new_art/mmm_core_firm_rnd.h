...
/*
Calcula as emissões de CO2 da empresa com base nas despesas de P&D e nas probabilidades de inovação e imitação.
*/
	v[0] = V("CO2_base");                              // Fator base de emissões de CO2 da empresa
	v[1] = V("Firm_RND_Expenses");                     // Despesas em P&D da empresa
	v[2] = V("sector_innovation_proportion");           // Proporção de P&D destinada à inovação
	v[3] = V("Firm_Innovation_Productivity");           // Produtividade alcançada com inovação
	v[4] = V("Firm_Imitation_Productivity");            // Produtividade alcançada com imitação
	v[5] = 100;                                        // Um valor máximo para normalizar o impacto do investimento em P&D

	// Cálculo da quantidade de recursos alocados para inovação e imitação
	v[6] = v[1] * v[2];                               // Recursos alocados para inovação
	v[7] = v[1] * (1 - v[2]);                         // Recursos alocados para imitação

	// Cálculo das probabilidades de sucesso
	v[8] = 1 - exp(-v[6]);                            // Probabilidade de sucesso da inovação
	v[9] = 1 - exp(-v[7]);                            // Probabilidade de sucesso da imitação

	// Cálculo das emissões de CO2
	v[10] = v[0] * (1 - (v[1] / v[5])) * (1 - v[8]) * (1 - v[9]);

	v[10] = max(0, v[10]);                           // Garante que as emissões não sejam negativas
RESULT(v[10])
...
/*
Classifica a empresa como "verde" (0) ou "marrom" (1) com base em suas emissões de CO2.
*/

	v[0] = V("Firm_CO2_Emissions");   // Emissões de CO2 calculadas
	v[1] = V("CO2_threshold");        // Limite base para classificações

	// Se as emissões de CO2 forem superiores ao limite, classifique como "marrom" (1)
	// Caso contrário, classifique como "verde" (0)
	if (v[0] > v[1]) 
		v[2] = 1;   // Empresa é marrom
	else 
		v[2] = 0;   // Empresa é verde
